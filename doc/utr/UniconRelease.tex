...
\date{September 28{\small $^{\textstyle th}$} 2020}% ----- Second public version
%\date{\today}
\begin{document}
\maketitle
...
\texttt{src/h/version.h}, which will be the version reported on the master
branch; \Bf starts at \Tag*{0} and will increment by one on
...
\MM without the trailing \Tag*{.0}. \Bf will only show up on release branches
that have been subsequently modified.

Although ``\Bf*'' implies that the reason for a subsequent release based on
\MM is merely to correct errors, it could also contain minor features that are
backported to an earlier release for distribution purposes. There is some further
discussion later on about the implications of this.

Later on, we present two slightly different proposals for the version string
representing the (slightly different) preferences of the authors. Rather than
continue to try to find a compromise between them, we invite the reader to
choose. Both of us would be happy with either alternative compared to the
present situation.
...
component numbers must alter. Our proposals are similar in spirit to [1] but
not as prescriptive.

The svn commit history was linear; after switching to git this is no longer
the case. However, if we zoom out, the commit history will still appear
linear in the large.  In any case, the release numbering scheme and the
release process itself must be able to deal with a general graph (of commits)
as well as a tree, or a simple sequence.
...
master branch. This is a common practice in software development, especially 
for projects with standard binary distributions. In time, the release tree
will thus come to resemble the iconic Saguaro cactus.

Note that the use of LTBs does not preclude cherry-picking commits from the
master branch into the LTB or vice-versa. In fact that would be a common
workflow -- a fix is applied to the master branch that is deemed to be important
enough to backport to earlier releases.
\pagebreak
...
\item If HEAD is untagged the version number will use the output of \texttt{git~describe},
  ...
  \begin{tabular}{lp{9cm}}
    \Tag{13.1}      &   is the name of the most recent release tag
     ...
     \Tag{943d2b72}&   is the short hash of the HEAD commit.
  \end{tabular}
\end{enumerate}
...
\begin{tabular}{ll|p{7cm}}
  ...
  \hline
  \Tag{13.2-176-g943d2b72} &  \Tag{07-Jun-2020} &
...
will usually see version strings like \Tag{13.1-176-g943d2b72}.
...
\subsection*{Release Details -- 2nd Alternative}
We propose that the version string be derived automatically from the git
