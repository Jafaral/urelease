...
\texttt{src/h/version.h} which will be the version reposrted on the master
...
backported to earlier release for distribution purposes. There is some further
...
The svn commit history was linear; after switching to git this is not the
case anymore. However, if we zoom out, the commit history will still appear
...
for projects with standard binary distribution. In time, the release tree
will thus come to resemble the iconic Saguaro cactus. Note that the use of
LTBs does not preclude cherry-picking commits from the master branch into
the LTB or vice-versa. In fact that would be the common workflow; a fix is 
applied to the master branch which is deemed to be important to backport to
earlier releases.
